...
ResultEntry TuningResultsManager::LookupWildcardFallback(
  ...
  const auto& km = kernel_map_it->second;
  bool found = false;
  std::string::difference_type best_wildcard_count = 0;
  std::string best_key;
  ResultEntry best_entry = ResultEntry::Null();
  for (const auto& [key, entry] : km) {
    if (key.find('*') == std::string::npos) {
      continue; // skip concrete entries
    }
    if (matches_wildcard_pattern(key, concrete_params_signature)) {
      const auto wildcard_count = std::count(key.begin(), key.end(), '*');
      if (!found || wildcard_count < best_wildcard_count ||
          (wildcard_count == best_wildcard_count && key < best_key)) {
        found = true;
